...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
   ...
   * @throws PropertyKeyNotFoundException Exception thrown if the message key is not found in the
   *     associated property file
   */
  private String getMessageInLanguage(String messageKey, String language)
    ...
    try {
      ...
      throw new PropertyKeyNotFoundException(
          "Property "
              + messageKey
              + " not found in messages_"
              + locale.toLanguageTag()
              + ".properties (or messages.properties if locale is French)");
    }
  }

  /**
   * Get a map containing, for each supported language, the localized associated message given a
   * messageKey
   *
  ...
  private Map<String, String> getLocalizedMessageMap(String messageKey) {
    ...
    for (String language : Arrays.asList("fr", "en")) {
      try {
        ...
        // If the message couldn't be loaded in a particular language, set the message to null for
        // that language
   ...
   *
   * @param localizedMessageMap Response body containing a map, with the message to display in each
   *     supported language
   * @param httpStatus Response status
   * @return ResponseEntity
   */
  private ResponseEntity<Object> buildResponseEntity(
      Map<String, String> localizedMessageMap, HttpStatus httpStatus) {
